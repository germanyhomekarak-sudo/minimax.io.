...
(function () {
  ...
  function ensureStyles() {
    ...
    s.textContent = `
      ...
      /* شريط أزرار الحفظ الثابت في أسفل لوحة المطور — يظهر دائماً */
      #bourjiLiveBar {
        position: sticky; bottom: 0; left: 0; right: 0; z-index: 10;
        background: linear-gradient(180deg, rgba(14,14,14,0) 0%, #0e0e0e 35%);
        padding: 14px 16px 12px; margin: 0 -16px -16px;
        display: flex; flex-wrap: wrap; gap: 8px; align-items: center;
        border-top: 1px solid rgba(201,164,92,.25);
        backdrop-filter: blur(6px);
      }
      #bourjiLiveBar .bourji-live-bar-label {
        font-size: 11px; color: #c9a45c; font-weight: 700;
        margin-inline-end: auto; letter-spacing: .2px;
      }
      #bourjiLiveBar button {
        padding: 9px 14px; border-radius: 8px; border: 0; cursor: pointer;
        font-family: Cairo, Tajawal, sans-serif; font-weight: 700; font-size: 13px;
        transition: transform .12s, box-shadow .15s;
      }
      #bourjiLiveBar button:hover { transform: translateY(-1px); }
      #bourjiLiveSaveBar {
        background: linear-gradient(135deg, #c9a45c, #b08c45); color: #0e0e0e;
        box-shadow: 0 4px 14px rgba(201,164,92,.35);
      }
      #bourjiLiveSaveBar:hover { box-shadow: 0 6px 18px rgba(201,164,92,.5); }
      #bourjiLiveResetBar {
        background: rgba(255,255,255,.06); color: #ddd; border: 1px solid rgba(255,255,255,.12) !important;
      }
    `;
  ...
  function injectSaveButtons() {
    const tryInject = () => {
      // [أ] شريط ثابت أسفل لوحة المطور — ظاهر مع كل التبويبات
      const devBody = $('.dev-body') || $('.dev-modal__panel');
      if (devBody && !$('#bourjiLiveBar')) {
        const bar = document.createElement('div');
        bar.id = 'bourjiLiveBar';
        bar.innerHTML = `
          <span class="bourji-live-bar-label">⚡ الحفظ المباشر</span>
          <button id="bourjiLiveSaveBar" type="button">💾 حفظ وتطبيق فوري</button>
          <button id="bourjiLiveResetBar" type="button">↩️ استعادة</button>
        `;
        devBody.appendChild(bar);
        $('#bourjiLiveSaveBar').onclick = saveAll;
        $('#bourjiLiveResetBar').onclick = resetAll;
      }

      // [ب] بلوك إضافي داخل تبويب "تصدير" (للتأكيد وللتوضيح)
      const exportPane = $('.dev-section[data-pane="export"]');
      if (exportPane && !$('#bourjiLiveSave')) {
        ...
        block.innerHTML = `
          ...
          <p class="bourji-live-note">
            ...
            لا حاجة لرفع أي ملف. <strong>يمكنك أيضاً استخدام شريط الحفظ الثابت في الأسفل.</strong>
            <br>• للتجربة السريعة: اضغط "حفظ وتطبيق فوري" (أسفل اللوحة).
      ...
      // اعتبرها "محقونة" إذا كان أحد العنصرين موجوداً
      return !!(($('#bourjiLiveBar')) || ($('#bourjiLiveSave')));
    };
